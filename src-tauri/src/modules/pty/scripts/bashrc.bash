# omnitab-shell-integration (bashrc)
#
...
if [ -z "$__OMNITAB_HOOKS_LOADED" ]; then
  ...
  _omnitab_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_omnitab_urlencode "$PWD")"
    if [ -z "$__OMNITAB_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __OMNITAB_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_omnitab_precmd:"*) ;;
    *) PROMPT_COMMAND="_omnitab_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
