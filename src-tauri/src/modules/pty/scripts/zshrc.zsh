# omnitab-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_omnitab_ret`.

{
  _omnitab_user_zdotdir="${OMNITAB_USER_ZDOTDIR:-$HOME}"
  [ -f "$_omnitab_user_zdotdir/.zshrc" ] && source "$_omnitab_user_zdotdir/.zshrc"
...
if [[ -z "$__OMNITAB_HOOKS_LOADED" ]]; then
  __OMNITAB_HOOKS_LOADED=1
  ...
  _omnitab_precmd() {
    local _omnitab_ret=$?
    printf '\e]133;D;%s\e\\' "$_omnitab_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_omnitab_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _omnitab_precmd
    add-zsh-hook preexec _omnitab_preexec
  fi
