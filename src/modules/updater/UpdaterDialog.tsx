...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S omnitab-bin";
    case "debian":
      return `sudo apt install ./OmniTab_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./OmniTab-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `OmniTab v${manual.version} is available`
                  : `OmniTab v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart OmniTab to finish installing."
              : downloading
                ? progress !== null
