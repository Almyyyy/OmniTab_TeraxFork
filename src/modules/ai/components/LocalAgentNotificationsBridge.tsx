...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "OmniTab needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "OmniTab run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "OmniTab finished", "Your task is ready");
    }
  }, [status, error]);
