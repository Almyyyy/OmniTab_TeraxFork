...
const INIT_PROMPT = `Scan this workspace and produce OMNITAB.md at the workspace root with:
...
Use grep/glob/list_directory/read_file to explore. Cap OMNITAB.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const OMNITAB_CMD_RE =
  /^<omnitab-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<omnitab-command name="${name}" />\n\n${prompt}`;
}
