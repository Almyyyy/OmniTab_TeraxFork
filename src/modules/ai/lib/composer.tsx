...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("omnitab:ai-attach-file", onAttach);
    return () => window.removeEventListener("omnitab:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the OMNITAB.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<omnitab-command name="${outcome.commandName}" />`;
        }
      }
    }
