...
OmniTab runs shells, reads/writes files, and talks to AI providers — so security bugs matter. If you find one, please tell us before posting it publicly.

## Reporting

Email **security@omnitab.app**. Include:
...
- Release artifacts on GitHub and `omnitab.app`
- The auto-updater
...
- **No telemetry.** OmniTab only talks to the network when you ask it to (AI requests, update checks, web preview).
...
- OmniTab runs whatever you (or the agent) tell it to run, with your permissions. That's kind of the point of a terminal.
- AI providers see whatever you send them. Read their retention policies.
- Local LLM endpoints (LM Studio, OpenAI-compatible) are trusted at the network level — only point OmniTab at servers you control.
